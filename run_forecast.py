...
USAGE WITH LIVE DATA:
    Set GM_API_BASE_URL (and GM_API_KEY etc — see APIDataSource in
    src/data_source.py) in the environment, then run this script normally.
    It will automatically pull from the live API instead of the Excel file.
...
def main():
    # --- 1. Load data — live API if GM_API_BASE_URL is set, else the Excel file ---
    source = APIDataSource() if os.environ.get("GM_API_BASE_URL") else FileDataSource(DATA_PATH)
